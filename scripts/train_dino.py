...
def validate(model, loader, device, cfg, epoch, wandb_run=None):
    ...
    for fire_hist, static, wind, targets, valid_t in pbar:
        ...
        # Log validation preview (first batch only)
        if wandb_run and (not logged_preview):
            import wandb
            import numpy as np
            max_images = 4
            ...
            preview_imgs = []
            for i in range(min(max_images, preds.shape[0])):
                # Get the last frame from fire history as base image
                T = int(valid_t[i].sum().item())
                last_fire = fire_hist[i, -1, 0].cpu().numpy()  # [H, W], binary {0, 1}

                # Convert to 3-channel grayscale for base image
                base_img = np.stack([last_fire] * 3, axis=-1)  # [H, W, 3]

                # Prepare masks (need integer labels)
                pred_mask = (preds[i, 0].cpu().numpy() > pred_thresh).astype(np.uint8)  # [H, W]
                target_mask = (targets[i, 0].cpu().numpy() > target_thresh).astype(np.uint8)  # [H, W]

                class_labels = {0: "background", 1: "fire"}

                preview_imgs.append(wandb.Image(
                    base_img,
                    masks={
                        "predictions": {"mask_data": pred_mask, "class_labels": class_labels},
                        "ground_truth": {"mask_data": target_mask, "class_labels": class_labels},
                    },
